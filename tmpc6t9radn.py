    ...
    def create_converted_entity(ag__, ag_source_map__, ag_module__):
        ...
        def tf__predict_function(iterator):
            ...
            with ag__.FunctionScope('predict_function', 'fscope', ag__.ConversionOptions(recursive=True, user_requested=True, optional_features=(), internal_convert_user_code=True)) as fscope:
                ...
                outputs = ag__.converted_call(self.distribute_strategy.run, (self.predict_step,), dict(args=(data,)), fscope)
                outputs = ag__.converted_call(reduce_per_replica, (outputs, self.distribute_strategy), dict(reduction='concat'), fscope)
                try:
                    do_return = True
        ...
        tf__predict_function.ag_source_map = ag_source_map__
        tf__predict_function.ag_module = ag_module__
        tf__predict_function = ag__.autograph_artifact(tf__predict_function)
